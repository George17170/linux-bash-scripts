#!/usr/bin/env bash
# docker_cleanup.sh
# Removes dangling images, stopped containers, unused volumes, and build cache.
# Safe to run regularly during local DevOps development to reclaim disk space.
#
# Usage:
#   ./docker_cleanup.sh [--dry-run] [--aggressive] [--yes]
#
# Modes:
#   default     Removes stopped containers, dangling images, unused volumes
#   --aggressive Also removes all unused images (not just dangling ones)
